...
app = FastAPI(title="TranscriptFlow API", version="3.0.0")
...
# المسار الدقيق والمطلق لملف الكوكيز على خوادم Render
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
COOKIE_FILE = os.path.join(BASE_DIR, "cookies.txt")

def get_cookie_path() -> Optional[str]:
    """التأكد من وجود الكوكيز بحجم فعلي"""
    if os.path.exists(COOKIE_FILE) and os.path.getsize(COOKIE_FILE) > 10:
        return COOKIE_FILE
    return None

def extract_video_id(url: str) -> Optional[str]:
    """استخراج ID الفيديو بدقة من أي صيغة رابط"""
    if not url:
        return None
    url = url.strip()
...
def extract_via_transcript_api(video_id: str) -> Optional[List[Dict[str, Any]]]:
    """المحاولة الأساسية والأسرع مع تمرير الكوكيز الرسمية"""
    cookie_path = get_cookie_path()
    raw_data = None

    try:
        # محاولة البحث عن الترجمة اليدوية أو التلقائية
...
def extract_via_ytdlp(video_id: str) -> Optional[List[Dict[str, Any]]]:
    """المحاولة الاحتياطية عبر yt-dlp مع ملف الكوكيز"""
...
def get_transcript(url: str = Query(..., description="YouTube video URL or Video ID")):
    ...
    # 1. التجربة الأولى السريعة مع الكوكيز
    clean_segments = extract_via_transcript_api(video_id)

    # 2. التجربة الاحتياطية
